...
import { Search, Github, ExternalLink, ChevronDown, X, Sun, Moon, Sparkles, TrendingUp, Shield, ArrowRight, GitBranch, GitCommit, GitMerge, Code2, Braces, Terminal } from 'lucide-react';
...
// Tool Card for Hero Section
function ToolShowcase({ tool, index }) {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <ArrowRight className={`w-4 h-4 flex-shrink-0 ${colors.text} opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all`} />
          </div>
...
// Project Card - Fixed overflow issue
  ...
  return (
    ...
    >
      <div className="p-4">
        ...
        {/* Tool Buttons - Fixed to prevent overflow */}
      ...
      <main className="relative z-10">
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6 pt-12 sm:pt-16 pb-10 sm:pb-12">
          <div className="max-w-4xl mb-10 sm:mb-12">
            ...
            <p className={`text-base sm:text-lg lg:text-xl leading-relaxed max-w-3xl ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              DECAL Lab's research tools help developers, researchers, and organizations understand,
              analyze, and sustain open source software ecosystems through AI-powered insights.
            </p>
          </div>

          {/* Tools Showcase */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-12 sm:mb-16">
            {tools.map((tool, index) => (
              <ToolShowcase key={tool.id} tool={tool} index={index} />
            ))}
          </div>
        </section>
        ...
        <section className={`border-t ${isDark ? 'border-white/[0.06] bg-slate-900/30' : 'border-slate-200 bg-slate-50/50'}`}>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12 sm:py-16">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 mb-6">
              <div>
                <h2 className={`text-xl sm:text-2xl lg:text-3xl font-bold mb-1.5 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  UC Davis Repositories
                </h2>
                <p className={`text-sm sm:text-base ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  Explore and analyze UC Davis open source projects with our research tools
                </p>
              </div>
            ...
            {/* Projects Count */}
            <p className={`text-xs sm:text-sm mb-4 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
        ...
        <footer className={`border-t ${isDark ? 'border-white/[0.06]' : 'border-slate-200'}`}>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 py-12 sm:py-16">
            ...
            <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-12 mb-10 sm:mb-12">
              ...
              {/* UC OSPO Network - Dark background for visibility in both themes */}
